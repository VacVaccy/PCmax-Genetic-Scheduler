...
pair<vector<vector<Gene>>, vector<int>> initialGeneration(const vector<int>& taskDurations, int populationsSize, int numMachines);
...
__global__ void initCurandStatesKernel(unsigned long long seed, int offset, int sequence_offset, curandState *states);
...
Config loadConfig(const string& configFile) {
    ...
    config.populationsSize = j.value("populationsSize", 200);        
...
vector<Gene> greedy(int numMachines, const vector<int>& taskDurations) {
    ...
    vector<long long> machinesLoad(numMachines, 0); 
    vector<Gene> chromosome;
...
int fitnessCalculation(int numMachines, const vector<Gene>& chromosome, const vector<int>& taskDurations) {
    vector<long long> timesList(numMachines, 0); 
...
void runGeneticAlgorithmOnGPU(Config config, int numMachines, const vector<int>& taskDurations, BestChromosome& bestChromosome) {
    ...
    if (numMachines > MAX_MACHINES_PER_CHROMOSOME) {
        cerr << "Error: numMachines (" << numMachines << ") exceeds MAX_MACHINES_PER_CHROMOSOME ("
             << MAX_MACHINES_PER_CHROMOSOME << "). Please increase MAX_MACHINES_PER_CHROMOSOME." << endl;
        exit(EXIT_FAILURE);
    }
    ...
    for (long long generation = 1; generation <= config.generations; ++generation) {
        auto currentTime = chrono::steady_clock::now();
        chrono::duration<double> elapsedTime = currentTime - globalStartTime;

        if (elapsedTime.count() >= config.maxTime) {
            cout << "\nTime limit reached! Terminating GA." << endl;
            break;
        }

        if (generation % 1000 == 0 || generation == 1) { 
            cout << "Generation " << generation
                 << " | Best Cmax: " << currentBestFitness
                 << " | Elapsed Time: " << fixed << setprecision(2) << elapsedTime.count() << "s" << endl;
        }
        ...
        crossoverKernel<<<crossover_blocks, crossover_threads>>>(
            d_population, d_offspring, numTasks, populationSize, d_randStates);
        CUDA_CHECK(cudaDeviceSynchronize()); 

        dim3 mutation_blocks((numOffspring + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK);
        dim3 mutation_threads(THREADS_PER_BLOCK);
        mutationKernel<<<mutation_blocks, mutation_threads>>>(
            d_offspring, d_taskDurations, numMachines, numTasks, numOffspring,
        ...
        calculateFitnessKernel<<<fitness_blocks, fitness_threads>>>(
            d_offspring, d_taskDurations, d_fitness, numMachines, numTasks, numOffspring);
        CUDA_CHECK(cudaDeviceSynchronize()); 
...
__global__ void curand_init_kernel(unsigned long long seed, int offset, int sequence_offset, curandState *states) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    curand_init(seed, idx + offset, sequence_offset, &states[idx]);
}
